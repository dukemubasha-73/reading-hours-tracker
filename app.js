(function () {
  ...
  // Streak Minimum: the hours a day needs (in total, across all sessions
  // that day) to count toward the streak. Independent of GOAL_KEY above —
  // Daily Goal stays purely aspirational (progress bar + celebration);
  // this is the actual streak-continuation threshold. Approach A (see
  // chat): a single global value, streaks are always computed live
  // against it, so changing it recalculates current/best streak using the
  // new value for all of history (not just future days).
  const STREAK_MINIMUM_KEY = "readingHoursStreakMinimum";
  const STREAK_MINIMUM_MIN = 0.25;
  const STREAK_MINIMUM_MAX = 24;
  ...
  // (Legacy — the collapsible cards were consolidated into a single graph
  // card with a type toggle; kept only so the old stored value isn't
  // reused for something else.)
  const HIGHEST_VS_CHART_VISIBLE_KEY = "readingHoursHighestVsChartVisible";
  // Remembers which graph type is showing in the consolidated Performance
  // graph card (see loadPerformanceChartType() above).
  const PERFORMANCE_CHART_TYPE_KEY = "readingHoursPerformanceChartType";
  const VALID_PERFORMANCE_CHART_TYPES = ["overall", "subject", "highestVs"];
  const DEFAULT_PERFORMANCE_CHART_TYPE = "overall";
  ...
  // Default Streak Minimum on first run: 50% of the current Daily Goal,
  // rounded to the nearest 0.25, floored at STREAK_MINIMUM_MIN. Takes
  // goalHours as a param (rather than reading the module-level variable)
  // so it can be called correctly regardless of load order.
  function defaultStreakMinimumFor(goal) {
    const rounded = Math.round((goal * 0.5) / 0.25) * 0.25;
    return roundTo(Math.min(STREAK_MINIMUM_MAX, Math.max(STREAK_MINIMUM_MIN, rounded)), 2);
  }

  // Unlike loadGoalHours (which always has a fixed DEFAULT_GOAL_HOURS to
  // fall back to), a missing Streak Minimum needs goalHours to compute its
  // default — so this must be called after goalHours is loaded, and it
  // persists the computed default immediately (first-load migration) so
  // it's stable from then on rather than silently recomputing forever.
  function loadStreakMinimum(goal) {
    const raw = localStorage.getItem(STREAK_MINIMUM_KEY);
    const val = parseFloat(raw);
    if (!isNaN(val) && val >= STREAK_MINIMUM_MIN && val <= STREAK_MINIMUM_MAX) return val;
    const def = defaultStreakMinimumFor(goal);
    saveStreakMinimum(def);
    return def;
  }

  function saveStreakMinimum(hours) {
    localStorage.setItem(STREAK_MINIMUM_KEY, String(hours));
    scheduleAutoSave();
  }
  ...
  // Remembers which graph type ("overall" | "subject" | "highestVs") is
  // showing in the consolidated Performance graph card. No saved value yet
  // (new users, or existing users who haven't touched the new toggle)
  // falls back to "overall", matching the section's historical default.
  function loadPerformanceChartType() {
    try {
      const raw = localStorage.getItem(PERFORMANCE_CHART_TYPE_KEY);
      return VALID_PERFORMANCE_CHART_TYPES.indexOf(raw) !== -1 ? raw : DEFAULT_PERFORMANCE_CHART_TYPE;
    } catch (e) { return DEFAULT_PERFORMANCE_CHART_TYPE; }
  }

  function savePerformanceChartType(type) {
    try { localStorage.setItem(PERFORMANCE_CHART_TYPE_KEY, type); } catch (e) { /* no-op */ }
  }
  ...
  let currentPerformanceChartType = loadPerformanceChartType(); // "overall" | "subject" | "highestVs"
  let overallChartInstance = null;
  let subjectChartInstance = null;
  let highestVsChartInstance = null;

  let goalHours = loadGoalHours();
  let streakMinimum = loadStreakMinimum(goalHours);
  // Seeded from today's ACTUAL stored total (not a hardcoded false) so the
  // very first renderGoalProgress() call — at startup, via refreshAll()
  // below — has something real to compare against. Without this, reopening
  // the app after already hitting today's goal earlier would read as a
  // false→true crossing on that first render and replay the celebration
  // (pulse + firework burst) on every single page load, not just the
  // moment the goal is actually newly completed. computeDailyTotals() is a
  // hoisted function declaration, so it's safely callable this early even
  // though it's defined further down the file.
  let wasGoalComplete = ((computeDailyTotals()[toISODate(new Date())] || 0) * 60) >= (goalHours * 60);
  ...
  const entriesToggle = document.getElementById("entriesToggle");
  const entriesContent = document.getElementById("entriesContent");
  const entriesChevron = document.getElementById("entriesChevron");
  ...
  const chartTypeButtons = document.querySelectorAll("#chartTypeSelector .chart-type-btn");
  ...
  const streakMinInput = document.getElementById("streakMinInput");
  ...
  const goalMetaLine = document.getElementById("goalMetaLine");
  const goalFirework = document.getElementById("goalFirework");
  ...
  const examResultsToggle = document.getElementById("examResultsToggle");
  const examResultsContent = document.getElementById("examResultsContent");
  const examResultsChevron = document.getElementById("examResultsChevron");

  // Performance charts: one consolidated graph card with a type toggle
  // (Overall / By Subject / Highest vs You). All three render functions
  // below share the same wrap/canvas — only the active type's controls
  // are shown, and only its render function actually draws.
  const performanceChartWrap = document.getElementById("performanceChartWrap");
  const performanceChartTypeButtons = document.querySelectorAll("#performanceChartTypeSelector .chart-type-btn");
  const performanceChartTitleEl = document.getElementById("performanceChartTitle");
  const performanceChartIconEl = document.getElementById("performanceChartIcon");
  const PERFORMANCE_CHART_META = {
    overall: { label: "Overall Performance", icon: "icon-trending-up" },
    subject: { label: "By Subject", icon: "icon-bars" },
    highestVs: { label: "Highest vs You", icon: "icon-trending-up" }
  };

  const overallChartWrap = performanceChartWrap;
  ...
  const overallRangeSelectorEl = document.getElementById("overallRangeSelector");
  const overallRangeButtons = document.querySelectorAll("#overallRangeSelector .range-btn");

  const subjectChartWrap = performanceChartWrap;
  ...
  const subjectChartControls = document.getElementById("subjectChartControls");
  ...
  const highestVsChartWrap = performanceChartWrap;
  ...
  // "All Entries" (Study) and "All Results" (Performance) can each get long,
  // so both are collapsible via the same reusable pattern — collapsed by
  // default, state remembered per-section in localStorage.
  const entriesListToggle = makeCardToggle(entriesToggle, entriesContent, entriesChevron, "readingHoursEntriesVisible");
  const examResultsListToggle = makeCardToggle(examResultsToggle, examResultsContent, examResultsChevron, "readingHoursExamResultsVisible");
  ...
  // longest run of consecutive days meeting the Streak Minimum, so "best
  // streak" survives even after the current streak is eventually broken.
  //
  // threshold defaults to the current streakMinimum but can be overridden
  // (see previewStreaksWithMinimum below), so the same logic can compute
  // "what would best streak be under a candidate new value" without
  // mutating any state.
  function computeBestStreak(threshold) {
    const min = typeof threshold === "number" ? threshold : streakMinimum;
    ...
    const dates = Object.keys(totals)
      .filter(function (d) { return totals[d] >= min && d <= todayStr; })
      .sort();
  ...
  // Computes the "Last 7 Days" total and average shown on the Study stat
  // cards. Both are derived from the 7 calendar days ending *yesterday* —
  // today is deliberately excluded, not because it necessarily has zero
  // hours, but because it's still in progress and may not yet contain a
  // complete set of entries. Including a partial day would distort the
  // figure. Today's hours become eligible for this window starting
  // tomorrow, once today has fully concluded (and the oldest day in the
  // current window drops off).
  //
  // Average uses only the concluded days within that window that actually
  // have entries (matches the app's existing averaging style elsewhere,
  // e.g. the old all-time avg divided by dates.length rather than by
  // calendar days) — a concluded day with zero hours logged doesn't count
  // toward the total, but it also isn't the same as today's excluded,
  // incomplete "invalid" day; it's simply skipped from the denominator.
  function computeLast7DaysStats(totals) {
    const todayStr = toISODate(new Date());
    let total = 0;
    ...
    cursor.setDate(cursor.getDate() - 1); // start at yesterday, skip today entirely

    for (let i = 0; i < 7; i++) {
      const key = toISODate(cursor);
      if (key !== todayStr && totals[key] && totals[key] > 0) {
        total += totals[key];
        loggedDayCount++;
      }
    ...
    const avg = loggedDayCount ? total / loggedDayCount : 0;
    return { total: roundTo(total, 2), avg: roundTo(avg, 2) };
  }

  // threshold optionally overrides streakMinimum for both the current-
  // streak loop and computeBestStreak (see previewStreaksWithMinimum).
  function computeStats(threshold) {
    const min = typeof threshold === "number" ? threshold : streakMinimum;
    const totals = computeDailyTotals();
    const dates = Object.keys(totals);
    // All-time total — kept as-is and used only for the total-hours
    // milestone toasts (checkAndAwardMilestones), NOT for display on the
    // stat card anymore. See computeLast7DaysStats() for what's shown.
    const total = dates.reduce(function (sum, d) { return sum + (totals[d] || 0); }, 0);

    const last7 = computeLast7DaysStats(totals);

    let streak = 0;
    let cursor = new Date();
    cursor.setHours(0, 0, 0, 0);

    if (!(toISODate(cursor) in totals) || totals[toISODate(cursor)] < min) {
      cursor.setDate(cursor.getDate() - 1);
    }

    while (true) {
      const key = toISODate(cursor);
      if (totals[key] && totals[key] >= min) {
        streak++;
        cursor.setDate(cursor.getDate() - 1);
      } else {
        break;
    ...
    return {
      ...
      last7Total: last7.total,
      last7Avg: last7.avg,
      streak: streak,
      best: computeBestStreak(min)
    };
  }

  // Pure preview: what would current/best streak be under a candidate
  // Streak Minimum, without touching any state. Used by the goal-edit
  // confirmation dialog to show real before/after numbers.
  function previewStreaksWithMinimum(candidateMinimum) {
    const s = computeStats(candidateMinimum);
    return { streak: s.streak, best: s.best };
  }

  function renderStats() {
    const s = computeStats();
    statTotal.textContent = s.last7Total;
    statAvg.textContent = s.last7Avg;
  ...
  function renderGoalProgress() {
    ...
    if (isComplete && !wasGoalComplete) {
      ...
      if (goalFirework) {
        goalFirework.classList.remove("burst");
        void goalFirework.offsetWidth; // force reflow so it restarts on repeat completions
        goalFirework.classList.add("burst");
      }
    }
    ...
    // Streak Minimum is a separate, independent threshold from Daily Goal
    // (see STREAK_MINIMUM_KEY above) — shown alongside it but never used
    // for the progress bar/celebration above, which stay goal-only.
    if (goalMetaLine) {
      goalMetaLine.textContent = "Goal: " + formatMinutesShort(goalMinutes) + " · Streak needs: " + formatMinutesShort(streakMinimum * 60);
    }
    // Optional visual state (see UI spec): today's fill communicates
    // "streak safe but goal not yet complete" as a distinct state from
    // full goal-complete, so the two thresholds read as genuinely
    // different milestones rather than one blurring into the other.
    goalProgressFill.classList.toggle("streak-safe", !isComplete && todayMinutes >= (streakMinimum * 60));
  }
  ...
  function updateStreakRiskBanner() {
    ...
    const totals = computeDailyTotals();
    const todayHours = totals[todayKey] || 0;
    // "At risk" now means today hasn't yet met the Streak Minimum (not
    // just "nothing logged") — a day with some hours logged but still
    // under the minimum still breaks the streak if left as-is.
    const metMinimumToday = todayHours >= streakMinimum;
    const stats = computeStats();
    const dismissedToday = sessionStorage.getItem(DISMISS_KEY) === todayKey;

    const atRisk = now.getHours() >= RISK_HOUR && !metMinimumToday && stats.streak > 0 && !dismissedToday;

    streakRiskBanner.classList.toggle("show", atRisk);
    if (atRisk) {
      const remainingHours = roundTo(Math.max(streakMinimum - todayHours, 0), 2);
      const remainingText = formatMinutesShort(remainingHours * 60);
      streakRiskText.textContent = "You need " + remainingText + " today to keep your " + stats.streak + "-day streak";
    }
  }
  ...
  // Shared "oldest → newest" comparator for exam results (left-to-right on
  // every Performance chart, matching the Study chart's convention). Ties
  // on date fall back to insertion order.
  function compareExamResultsAsc(a, b) {
    if (a.date !== b.date) return a.date < b.date ? -1 : 1;
    return (a.createdAt || 0) - (b.createdAt || 0);
  }

  // Returns exam results sorted oldest → newest, optionally limited to the
  // last N by currentOverallRange.
  function getOverallChartData() {
    const sorted = examResults.slice().sort(compareExamResultsAsc);
  ...
  // Shared "MMM D" x-axis label formatter used by every Performance line
  // chart (Overall / By Subject / Highest vs You).
  function formatChartDateLabels(data) {
  ...
  // Shared Chart.js option shape for every 0–100% Performance line chart:
  // same axis bounds, tick/grid theming, and hover behavior — only the
  // tooltip label callback differs per chart.
  function buildPerformanceChartOptions(themeColors, tooltipLabelCallback) {
    return {
      responsive: true,
      maintainAspectRatio: false,
      interaction: { mode: "index", intersect: false },
  ...
  function renderOverallChart() {
    ...
    if (currentPerformanceChartType !== "overall") return;
    ...
    const ctx = ensureChartCanvas(overallChartWrap, "performanceChart");

    const labels = formatChartDateLabels(data);
    ...
    const options = buildPerformanceChartOptions(themeColors, function (ctx) {
  ...
  function renderSubjectChart() {
    ...
    if (currentPerformanceChartType !== "subject") return;
    ...
    const ctx = ensureChartCanvas(subjectChartWrap, "performanceChart");
  ...
  // "Highest vs You": only has data once a user starts filling in the
  // optional Highest Score field — one of the three graph types offered
  // by the toggle below, alongside Overall and By Subject.
  ...
  function renderHighestVsChart() {
    ...
    if (currentPerformanceChartType !== "highestVs") return;
    ...
    const ctx = ensureChartCanvas(highestVsChartWrap, "performanceChart");
  ...
  // ---------------------------------------------------------------------
  // Performance graph type toggle (Overall / By Subject / Highest vs You)
  // ---------------------------------------------------------------------

  // Syncs the toggle buttons' active state, the shared header title/icon,
  // and which type-specific controls (range selector, subject picker,
  // legends) are shown for the given type. Does not render or destroy any
  // chart — see setActivePerformanceChartType() for that.
  function updatePerformanceChartTypeUI(type) {
    performanceChartTypeButtons.forEach(function (b) {
      b.classList.toggle("active", b.dataset.perfChartType === type);
    });

    const meta = PERFORMANCE_CHART_META[type];
    if (meta) {
      if (performanceChartTitleEl) performanceChartTitleEl.textContent = meta.label;
      if (performanceChartIconEl) performanceChartIconEl.setAttribute("href", "#" + meta.icon);
    }

    if (overallRangeSelectorEl) overallRangeSelectorEl.style.display = (type === "overall") ? "" : "none";
    if (overallChartRangeEl) overallChartRangeEl.style.display = (type === "overall") ? "" : "none";
    if (overallChartLegendEl) overallChartLegendEl.style.display = (type === "overall") ? "" : "none";

    if (subjectChartControls) subjectChartControls.style.display = (type === "subject") ? "" : "none";
    if (subjectChartRangeEl) subjectChartRangeEl.style.display = (type === "subject") ? "" : "none";

    if (highestVsChartRangeEl) highestVsChartRangeEl.style.display = (type === "highestVs") ? "" : "none";
    if (highestVsChartLegendEl) highestVsChartLegendEl.style.display = (type === "highestVs") ? "" : "none";
  }

  // The three chart types share one canvas — whichever *ChartInstance
  // isn't for the newly active type must be destroyed before that type's
  // render function reuses the canvas, or Chart.js throws on re-creation.
  function destroyInactivePerformanceCharts(activeType) {
    if (activeType !== "overall" && overallChartInstance) { overallChartInstance.destroy(); overallChartInstance = null; }
    if (activeType !== "subject" && subjectChartInstance) { subjectChartInstance.destroy(); subjectChartInstance = null; }
    if (activeType !== "highestVs" && highestVsChartInstance) { highestVsChartInstance.destroy(); highestVsChartInstance = null; }
  }

  function renderActivePerformanceChart() {
    if (currentPerformanceChartType === "overall") renderOverallChart();
    else if (currentPerformanceChartType === "subject") renderSubjectChart();
    else if (currentPerformanceChartType === "highestVs") renderHighestVsChart();
  }

  function setActivePerformanceChartType(type) {
    if (VALID_PERFORMANCE_CHART_TYPES.indexOf(type) === -1 || type === currentPerformanceChartType) return;
    currentPerformanceChartType = type;
    savePerformanceChartType(type);
    destroyInactivePerformanceCharts(type);
    updatePerformanceChartTypeUI(type);
    renderActivePerformanceChart();
  ...
  performanceChartTypeButtons.forEach(function (btn) {
    btn.addEventListener("click", function () {
      setActivePerformanceChartType(btn.dataset.perfChartType);
    });
  });

  // Sync the toggle/controls to whichever type was loaded from storage (or
  // the "overall" default) before any data has rendered — refreshAll()'s
  // render calls below will only draw the active type's chart.
  updatePerformanceChartTypeUI(currentPerformanceChartType);
  ...
  function parseImportFile(text, filename) {
    ...
    let importedStreakMinimum = null;
    ...
    if (looksLikeJson) {
      ...
      if (parsed && typeof parsed === "object" && parsed.sessions && typeof parsed.sessions === "object") {
        ...
        if (typeof parsed.streakMinimum === "number" && parsed.streakMinimum >= STREAK_MINIMUM_MIN && parsed.streakMinimum <= STREAK_MINIMUM_MAX) {
          importedStreakMinimum = parsed.streakMinimum;
        }
    ...
    return { sessions: sessionsOut, examResults: examResultsOut, goalHours: importedGoal, streakMinimum: importedStreakMinimum, milestonesAwarded: importedMilestones };
  }
  ...
  function performImport(parsed, replace) {
    ...
    if (typeof parsed.streakMinimum === "number" && parsed.streakMinimum >= STREAK_MINIMUM_MIN && parsed.streakMinimum <= STREAK_MINIMUM_MAX) {
      streakMinimum = roundTo(parsed.streakMinimum, 2);
      saveStreakMinimum(streakMinimum);
    }
  ...
  // Daily goal + Streak Minimum share one inline editor (gear icon reveals
  // both fields). Save validates and persists both; Cancel just hides the
  // row again without changing anything. The two are independent settings
  // — see STREAK_MINIMUM_KEY above — validated and (when the minimum
  // actually changes streak outcomes) confirmed separately below.
  goalEditBtn.addEventListener("click", function () {
    goalInput.value = goalHours;
    streakMinInput.value = streakMinimum;
  ...
  // Applies an already-validated, already-confirmed-if-needed Streak
  // Minimum: persists it and refreshes everything derived from it (stats,
  // risk banner, goal meta line).
  function applyStreakMinimum(newValue) {
    streakMinimum = newValue;
    saveStreakMinimum(streakMinimum);
    refreshAll();
  }
  ...
  goalSaveBtn.addEventListener("click", function () {
    ...
    const streakValue = parseFloat(streakMinInput.value);
    if (isNaN(streakValue) || streakValue < STREAK_MINIMUM_MIN) {
      showToast("Streak minimum must be at least " + STREAK_MINIMUM_MIN + "h.", true);
      return;
    }
    if (streakValue > STREAK_MINIMUM_MAX) {
      showToast("Streak minimum can't exceed 24 hours.", true);
      return;
    }

    // Daily Goal applies immediately regardless of what happens with the
    // Streak Minimum below — the two are independent (see spec).
    goalHours = roundTo(goalValue, 2);
    saveGoalHours(goalHours);
    goalEditRow.classList.remove("show");

    const newStreakMinimum = roundTo(streakValue, 2);
    if (newStreakMinimum === streakMinimum) {
      refreshAll();
    ...
    const after = previewStreaksWithMinimum(newStreakMinimum);

    if (before.streak === after.streak && before.best === after.best) {
      // Nothing about the actual streak numbers changes at this threshold
      // — save directly, no confirmation needed (per spec).
      applyStreakMinimum(newStreakMinimum);
      showToast("Daily goal and streak minimum updated");
      return;
    }

    openModal(
      "Update streak minimum?",
      "Changing the streak minimum will recalculate your streaks using the new value.\n\n" +
        "Current streak: " + before.streak + " → " + after.streak + " days\n" +
        "Best streak: " + before.best + " → " + after.best + " days",
      "Continue",
      function () {
        applyStreakMinimum(newStreakMinimum);
        showToast("Daily goal and streak minimum updated");
      ...
      function () {
        // Cancelled: Streak Minimum stays as it was. The Daily Goal change
        // above already applied and saved regardless — the two settings
        // are independent, so cancelling this dialog only backs out the
        // streak-minimum half of the edit.
        refreshAll();
        showToast("Daily goal updated");
      }
    );
  ...
  function applyTheme(theme) {
    ...
    // Keep the active Performance chart's colors in sync with the theme
    // too — it reads the same CSS custom properties as the Study chart.
    // Guarded the same way as the Study chart above: skip if nothing has
    // rendered there yet (e.g. during initial startup, before refreshAll()
    // has run for the first time).
    if (overallChartInstance || subjectChartInstance || highestVsChartInstance) {
      renderActivePerformanceChart();
    }
  }
  ...
  refreshAll();
  setActiveOverallRangeButton();
  // refreshAll() already renders the exam list + the active Performance
  // chart (see its body above, and updatePerformanceChartTypeUI() further
  // up which already synced the toggle/controls), so no separate
  // refreshPerformance() call is needed here — that used to re-run the
  // same list/chart render + a second localStorage write + a second pass
  // through cloudChangeListeners immediately after refreshAll() had just
  // done it, doubling startup cost for no visible effect.
  ...
  entriesListToggle.setVisible(entriesListToggle.loadVisible());
  examResultsListToggle.setVisible(examResultsListToggle.loadVisible());
  ...
  window.ReadingHoursApp = {
    ...
    setState: function (state) {
      ...
      if (typeof state.streakMinimum === "number" && state.streakMinimum >= STREAK_MINIMUM_MIN && state.streakMinimum <= STREAK_MINIMUM_MAX) streakMinimum = state.streakMinimum;
