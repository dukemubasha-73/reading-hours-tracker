(function () {
  ...
  function createSubjectManager(refs) {
    ...
    function render() {
      if (subjects.length === 0) {
        refs.listEl.innerHTML = '<div class="empty-state">No subjects yet. Add one below.</div>';
        return;
      }
      ...
      refs.listEl.querySelectorAll(".subject-edit-btn").forEach(function (btn) {
        btn.addEventListener("click", function () { startEdit(btn.dataset.id); });
      });
      refs.listEl.querySelectorAll(".subject-delete-btn").forEach(function (btn) {
        btn.addEventListener("click", function () { removeSubject(btn.dataset.id); });
      });
    }
  ...
  function renderEntriesList() {
    ...
    entriesList.querySelectorAll(".day-header").forEach(function (el) {
      el.addEventListener("click", function () {
        ...
        const sessionsEl = el.nextElementSibling;
        const toggleIcon = el.querySelector(".day-toggle");
  ...
  function renderChartLegend(subjectIds, targetElId) {
  ...
  function renderExamResultsList() {
    ...
    if (sorted.length === 0) {
      examResultsList.innerHTML = '<div class="empty-state">No results yet. Log your first exam above.</div>';
      return;
    }

    examResultsList.innerHTML = sorted.map(function (r) {
      const info = resolveSubject(r.subjectId);
    ...
    examResultsList.querySelectorAll(".exam-edit-btn").forEach(function (btn) {
      btn.addEventListener("click", function () { startEditExamResult(btn.dataset.id); });
    });
    examResultsList.querySelectorAll(".exam-delete-btn").forEach(function (btn) {
      btn.addEventListener("click", function () { deleteExamResult(btn.dataset.id); });
    });
  }
  ...
  function renderOverallChart() {
    ...
    if (data.length === 0) {
      ...
      overallChartWrap.innerHTML = '<div class="empty-state">No results yet. Log your first exam above to see your trend.</div>';
    ...
    const pointColors = data.map(function (r) { return resolveSubject(r.subjectId).color; });
  ...
  function renderSubjectChart() {
    ...
    if (!currentSubjectChartId) {
      ...
      subjectChartWrap.innerHTML = '<div class="empty-state">Add a subject first.</div>';
    ...
    if (data.length === 0) {
      if (subjectChartInstance) { subjectChartInstance.destroy(); subjectChartInstance = null; }
      subjectChartWrap.innerHTML = '<div class="empty-state">No results logged for ' + escapeHtml(info.name) + ' yet.</div>';
      return;
    }
  ...
  if (subjectChartToggle) {
    ...
    subjectChartToggle.addEventListener("keydown", function (e) {
      if (e.key === "Enter" || e.key === " ") {
        e.preventDefault();
        setSubjectChartVisible(subjectChartContent.style.display === "none");
      }
  ...
  function parseImportFile(text, filename) {
    ...
    function addExamResultRow(date, subjectName, percentage, examName, subjectId, createdAt) {
      if (!isValidDateStr(date)) return;
  ...
  function setActiveSection(section) {
    ...
    if (headerTitleEl) headerTitleEl.textContent = SECTION_CONTENT[section].title;
